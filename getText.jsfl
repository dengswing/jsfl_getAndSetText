...
function getElement(element)
{
	...
	fl.trace("cutout = "+cutOut(sValue)+" == "+sValue)
	if(cutOut(sValue)) return;
	
	var fId=filterRepeat(sValue,oStr);
	if(fId==-1)
	{
		iIndex++;					
		oStr[iIndex] = sValue;
	...
	//fl.trace(sValue+" ."+".."+fId)
	element.setTextString(sValue +"@&"+fId);
}


fl.trace("get finish"+stringify(oStr));
FLfile.write(outFolder+"text.json",stringify(oStr));

function filterRepeat(sValue,oData)
{
	var iIndex=-1;
	for(var i in oData)
	{
